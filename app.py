    ...
    if not selected_params:
        ...
        with st.spinner("⏳ Sedang memproses prediksi..."):
            ...
            with tab3:
                ...
                st.subheader("Unduh Gambar Prediksi")

                if forecast is not None and selected_params:
                    # Buat grafik gabungan seperti di tab 2
                    combined_fig = make_subplots(
                        rows=len(selected_params), cols=1, shared_xaxes=True,
                    ...
                    combined_fig.update_layout(
                        ...
                        title_text="Gabungan Grafik Prediksi",
                        showlegend=False
                    )

                    try:
                        # Render gambar ke PNG bytes
                        img_bytes = combined_fig.to_image(format="svg", width=1000, height=300 * len(selected_params))
                        buffer = io.BytesIO(img_bytes)

                        # Tombol untuk mengunduh gambar
                        st.download_button(
                            label="🖼️ Unduh Grafik Prediksi (SVG)",
                            data=buffer,
                            file_name="grafik_prediksi.svg",
                            mime="image/svg+xml"
                        )
                    ...
                    st.info("Silakan lakukan prediksi terlebih dahulu.")