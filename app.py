        ...
        with st.spinner("⏳ Sedang memproses prediksi..."):
            ...
            with tab3:
                ...
                if forecast is not None and selected_params:
                    ...
                    # 🔧 Konversi ke PNG & tombol unduh GAMBAR
                    try:
                        img_bytes = combined_fig.to_image(format="svg")  
                        st.download_button(
                        label="🖼️ Unduh Grafik Gambar",
                        data=img_bytes,
                        file_name="grafik_prediksi.svg",
                        mime="image/svg+xml"
                        )
                    except Exception as e:
                        st.error("Gagal membuat gambar. Grafik masih bisa dilihat, namun tidak dapat diunduh. Detail error:")
                        st.text(str(e))